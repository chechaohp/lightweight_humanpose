...
class HighResolutionModule_TypeA(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 num_channels, fuse_method, multi_scale_output=True):
        super(HighResolutionModule_TypeA, self).__init__()
        self._check_branches(
            num_branches, blocks, num_blocks, num_inchannels, num_channels)
...
class HHRNet(nn.Module):

    def __init__(self, cfg, **kwargs):
        
        self.extra = cfg.MODEL.EXTRA
        self.NO_STAGE = cfg.MODEL.EXTRA.NO_STAGE
        super(HHRNet, self).__init__()

        # stem net
        num_channel = self.extra.STEM_INPLANES
        self.conv1 = nn.Conv2d(3, num_channel, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(num_channel, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(num_channel, num_channel, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn2 = nn.BatchNorm2d(num_channel, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)

        self.inplanes = self.extra.STEM_INPLANES

        # Stage i
        for i in range(1, self.NO_STAGE+1):
            num_channels = self.extra['STAGE{}'.format(i)]['NUM_CHANNELS']
            block = blocks_dict[self.extra['STAGE{}'.format(i)]['BLOCK']]
            num_blocks = self.extra['STAGE{}'.format(i)]['NUM_BLOCKS']
            ...
            if i==1:
                ...
                setattr(self, 'transition{}'.format(i-1), 
                        ...
                        self._make_transition_layer(pre_stage_channels, num_channels))
                if i==self.NO_STAGE:
                    stage_i, pre_stage_channels = self._make_stage(self.extra['STAGE{}'.format(i)], num_channels, 
                                                                   multi_scale_output=False)
                else:
                    stage_i, pre_stage_channels = self._make_stage(self.extra['STAGE{}'.format(i)], num_channels)
                setattr(self, 'stage{}'.format(i), stage_i)
        ...
        self.num_deconvs = self.extra.DECONV.NUM_DECONVS
        self.deconv_config = self.extra.DECONV
        self.loss_config = cfg.LOSS

        self.pretrained_layers = self.extra['PRETRAINED_LAYERS']
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            ...
            for _ in range(self.extra.DECONV.NUM_BASIC_BLOCKS):
                layers.append(nn.Sequential(
    ...
    def forward(self, cfg, x):
        ...
        for i in range(2, self.NO_STAGE+1):
            x_list = []
            for i in range(self.extra['STAGE{}'.format(i)]['NUM_BRANCHES']):
                if getattr(self, 'transition{}'.format(i-1), None) is not None:
                    x_list.append(getattr(self, 'transition{}'.format(i-1))(x))
                else:
                    x_list.append(x)
